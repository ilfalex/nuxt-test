  ...
  computed: {
    isVerified () {
      ...
      for (x in this.verificationStatus) {
        ...
        if (this.verificationStatus[x].doc_status < 4) { return false }
      }
      return true
    }
  },
  beforeCreate () {
    ...
    user.getVerficiationStatus()
      .then(response => {
        ...
        // loop through the statuses and determine if all == 4
        for (const x in this.verificationStatus) {
          if (this.verificationStatus[x].doc_status < 4) { this.dialog = true }
        }
      })
  },
